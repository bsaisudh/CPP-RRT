...
void RRT::sampleFromCs() {
   ...
   */
  int random_location = std::rand()
      / ((RAND_MAX + 1u) / Map->configSpace.size());
...
void RRT::addToTree() {
  ...
  Tree.push_back( { { newPoint[0], newPoint[1], test, parentPointIndex } });
}
...
int RRT::computeNewPoint() {
  ...
  for (int i = 0; i < Tree.size(); i++) {
    distance.push_back(
        std::pow(
            std::pow(Tree.at(i)[0] - sampledPoint[0], 2)
                + std::pow(Tree.at(i)[1] - sampledPoint[1], 2),
            0.5));
  }
  int index = 0;
  ...
  int availableMovements[8][2] = { { Tree.at(index)[0], Tree.at(index)[1] + 1 },
      { Tree.at(index)[0] + 1, Tree.at(index)[1] + 1 }, { Tree.at(index)[0] + 1,
          Tree.at(index)[1] }, { Tree.at(index)[0] + 1, Tree.at(index)[1] - 1 },
      { Tree.at(index)[0], Tree.at(index)[1] - 1 }, { Tree.at(index)[0] - 1,
          Tree.at(index)[1] - 1 }, { Tree.at(index)[0] - 1, Tree.at(index)[1] },
  ...
  for (int j = 0; j < 8; j++) {
    distanceFromNewPoints.push_back(
        std::pow(
            std::pow(availableMovements[j][0] - sampledPoint[0], 2)
                + std::pow(availableMovements[j][1] - sampledPoint[1], 2),
            0.5));
  }
  int newPointIndex = 0;
...
std::vector<point> RRT::buildPath() {
  ...
  Tree.push_back( { { goalPoint[0], goalPoint[1], static_cast<int>(Tree.size()),
      parentPointIndex } });
